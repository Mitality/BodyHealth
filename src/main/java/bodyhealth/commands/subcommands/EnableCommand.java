    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        try {
            ...
            if (isSelf) {
                ...
                BodyHealthUtils.getBodyHealth(target).setEnabled(true);
                MessageUtils.notifySender(sender, Config.prefix + Lang.bodyhealth_enable_success_other.replace("{Player}", Objects.requireNonNull(target.getName())));
                if (target.isOnline()) MessageUtils.notifyPlayer(target.getPlayer(), Config.prefix + Lang.bodyhealth_enable_notification.replace("{Player}", sender.getName()));
            }
